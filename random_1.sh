...
while true; do
	...
	printf "$(basename "$c1"): ";
	./vm/corewar -dump $DUMP -v 31 $c1 > $f1;
	./docs/ressources/corewar -d $DUMP -v 31 $c1 > $f2;
	if [ "" == "$(diff $f1 $f2)" ]; then
		echo "\033[32;1mgood\033[0m";
	else
		echo "\033[31;1mbad\033[0m";
		echo "run \"\033[0;1mDUMP=${DUMP}; c1="$c1"; diff <(./vm/corewar -dump \$DUMP -v 31 \$c1) <(./docs/ressources/corewar -d \$DUMP -v 31 \$c1 \$c2)\033[0m\""
		echo "see ${f1} and ${f2}";
	fi
done
